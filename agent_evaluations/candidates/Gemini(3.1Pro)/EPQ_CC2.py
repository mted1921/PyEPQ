...
CHANGES
-------
- SCIPY-DEV-1: `integrate` delegates to `integrate_literal` because standard `scipy.integrate` 
  solvers do not map cleanly to the internal step-tracking state fields (`mNOk`, `mNBad`, `mNSaved`, etc.) 
  that are exposed by the Java API.
"""
...
class AdaptiveRungeKutta(abc.ABC):
    ...
    BUG_LEDGER: tuple = ()  # no bugs identified; Java source attached
    ...
    def getX(self, i: int) -> float:
        if self.mXSave is None:
            raise EPQException("No X values saved.")
        return float(self.mXSave[i])

    def getY(self, i: int) -> F64Array:
        if self.mYSave is None:
            raise EPQException("No Y values saved.")
        return self.mYSave[i, :self.mNVariables]
    ...
    def integrate(self, x1: float, x2: float, ystart: F64Array, eps: float, h1: float) -> F64Array:
        """
        Integrate the ODE specified by derivatives using the adaptive step size Runge-Kutta algorithm.
        
        SCIPY-DEV-1: scipy.integrate.solve_ivp does not identically trace steps and populate the
        internal step-tracking state fields (mNSaved, mXSave, etc.) exposed by the Java API. 
        Delegates to the literal port to maintain object invariants.
        """
