...
CHANGES
-------
* SCIPY-DEV-1: `integrate` is kept as a literal port without a scipy-backed replacement. 
  `scipy.integrate.solve_ivp` uses different internal stepping algorithms and cannot 
  faithfully populate this class's observable trajectory state (`mXSave`, `mYSave`, 
  `mNOk`, `mNBad`, `mHDid`) at the exact intervals expected by Java callers. An 
  `integrate_literal` alias is provided for completeness.
* R7: `Math.round(x)` in `integrate` converted to `int(math.floor(x + 0.5))`.
...
class AdaptiveRungeKutta(abc.ABC):
    ...
    BUG_LEDGER: tuple = ()  # no bugs identified; Java source accurately reflects NR Cash-Karp RK45
    ...
    def integrate(self, x1: float, x2: float, ystart: F64Array, eps: float, h1: float) -> F64Array:
        ...
        if self.mSaveInterval != sys.float_info.max:
            # R7: int(math.floor(val + 0.5)) for Math.round
    ...
    def integrate_literal(self, x1: float, x2: float, ystart: F64Array, eps: float, h1: float) -> F64Array:
        """Alias for `integrate` to satisfy the parity test naming requirement."""
        return self.integrate(x1, x2, ystart, eps, h1)

    @abc.abstractmethod
    def derivatives(self, x: float, y: F64Array, dydx: F64Array) -> None:
        """
        The derived class provides an implementation of the derivatives function.
        x & y[] are input and the user provided implementation of derivatives is 
        responsible for returning the derivatives in the array dydx. The lengths 
        of y and dydx are equal to mNDimensions.
        """
        pass