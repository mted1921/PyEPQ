...
CHANGES
-------
* SCIPY-DEV-1: In `integrate`, delegated to `integrate_literal`. 
  `scipy.integrate.solve_ivp` uses Dormand-Prince instead of Cash-Karp and does not exactly reproduce 
  the step-by-step state (`mNOk`, `mNBad`, `mXSave`) of the Java implementation.
"""
...
class AdaptiveRungeKutta(abc.ABC):
    ...
    BUG_LEDGER: tuple = ()  # no bugs identified; Java source did not present obvious errors
    ...
    def _qcStep(self, x: float, y: F64Array, dydx: F64Array, htry: float, eps: float, yscal: F64Array) -> float:
        ...
        while True:
            ...
            if errmax > 1.0:
                ...
                xnew = x + h
                if xnew == x:
                    raise EPQException("Step size underflow in AdaptiveRungeKutta._qcStep.")
            if errmax <= 1.0:
                break
        ...
        self.mHDid = h
        x += h
        y[:self.mNVariables] = self._mQcYTemp[:self.mNVariables]
        return x
