...
Each run writes its terminal output to tests/archive/<name>.txt where <name>
is derived from the test file(s) being run:
  • Single file  → stem of that file  (e.g. test_parity_math2_ver1_1_3)
  • Multi-file   → suite_<YYYYMMDD_HHMMSS>
...
Also tees all terminal output to archive/<name>.txt (ANSI codes stripped) so
...
def _derive_archive_name(config) -> str:
    """Return a filename stem for the archive output of this pytest run.

    Single test file → that file's stem.
    Multi-file or whole-suite → suite_YYYYMMDD_HHMMSS.
    """
    args = getattr(config, 'args', [])
    stems = []
    for a in args:
        # Strip node-ID suffixes (::Class::method) before checking extension
        path_part = a.split('::')[0]
        if path_part.endswith('.py'):
            stems.append(Path(path_part).stem)
    if len(stems) == 1:
        return stems[0]
    stamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    return f"suite_{stamp}"
...
def pytest_sessionstart(session):
    """Tee TerminalReporter output to archive/<name>.txt.

    <name> is the stem of the single test file being run, or
    suite_<timestamp> when multiple files are collected.
    """
    archive_dir = Path(__file__).parent / "archive"
    archive_dir.mkdir(exist_ok=True)
    name = _derive_archive_name(session.config)
    out_path = archive_dir / f"{name}.txt"
    try:
