r"""
test_parity_findroot_ver1_1_1.py -- parity harness for FindRoot

Revision ver1_1_1 (2026-06-25): P3 fix (gen1_review) — the private evaluation
counter is accessed as the R1-prefixed `_mNEvals`, not the bare Java name
`mNEvals`. The port follows CONVERSION_GUIDE R1 (`private int mNEvals` ->
`self._mNEvals`); see TESTING_GUIDE "Accessing port internals in tests". The
public `EvaluationCount()` checks are unchanged. No other behavioural change.
...
from FindRoot_ver2_1_0 import FindRoot as PyFindRoot
...
class TestFindRootParity:
    """Compare Java FindRoot.perform with Python FindRoot.perform.
