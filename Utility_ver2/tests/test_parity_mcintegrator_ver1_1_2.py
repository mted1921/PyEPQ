r"""
test_parity_mcintegrator_ver1_1_2.py — parity harness for MCIntegrator

Revision ver1_1_2 (2026-06-25): P10 (gen1_review). The empty-domain edge cases
(TestEdgeCases / TestHypothesis) require `compute()` to return a ZERO VECTOR, not
None — callers subscript the result unconditionally. The harness already encoded
that convention; the matching port fix (MCIntegrator FIX-1) makes `compute()`
return `[0.0] * dim` on an empty domain. No test-body change beyond the updated
port import.
...
from MCIntegrator_ver2_1_0 import MCIntegrator as PyMCIntegrator
