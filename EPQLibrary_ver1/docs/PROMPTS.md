...
2. `<Subpkg>_LEDGER.md` — to identify sibling port filenames for the dependency table
...
## Java class
`gov.nist.microanalysis.EPQLibrary.<CLASS_NAME>`

Source: `src/gov/nist/microanalysis/EPQLibrary/<CLASS_NAME>.java`
...
6. `<Subpkg>_LEDGER.md` — **required when the Java class imports other EPQLibrary classes**
...
the Python port file. Do **not** write any test or harness files. Limit the context for each conversion to the ctx folder (src\gov\nist\microanalysis\PyEPQ\<Subpkg>\ctx), as well as the source Java (src\gov\nist\microanalysis\EPQLibrary), and corresponding spec (src\gov\nist\microanalysis\PyEPQ\<Subpkg>\spec). Perform each port conversion in a sequential manner, avoiding paralellization. 
...
```python
r"""
<CLASS_NAME>_ver1_1_0.py — Python port of gov.nist.microanalysis.EPQLibrary.<CLASS_NAME>
...
------------------------------------------------------------------------
Original Java source (gov.nist.microanalysis.EPQLibrary.<CLASS_NAME>)
------------------------------------------------------------------------
/**
...
try:
    ...
    try:
        ...
        from gov.nist.microanalysis.PyEPQ.<Subpkg>._epq_compat import EPQException, F64Array, JavaRandom  # type: ignore
```
...
- Sibling module filenames taken from `<Subpkg>_LEDGER.md` "Port file" column — never guessed
...
# NEVER use the full dotted path (gov.nist.microanalysis.PyEPQ.<Subpkg>.<CLASS_NAME>_ver...).
# _parity_lib.py adds PyEPQ/<Subpkg>/ to sys.path on import, so the bare module
# name above is the ONLY correct form. The full path causes "No module named 'gov'"
# because sys.path contains <Subpkg>/, not the repo root.
ctx = setup_parity("gov.nist.microanalysis.EPQLibrary.<CLASS_NAME>")
...
- [ ] Port module imported with the **bare module name** (`from <CLASS_NAME>_ver1_1_0 import ...`), never with the full dotted path (`gov.nist.microanalysis.PyEPQ.<Subpkg>.<CLASS_NAME>_ver1_1_0`). The three-tier import pattern in CONVERSION_GUIDE R3 is for port files importing `_epq_compat` — it does NOT apply here.
...
Review the compliance report (src\gov\nist\microanalysis\PyEPQ\tools\reports\compliance_<Subpkg>.md) and determine which ports need repair. You will find detailed test results for each port in the results archive (src\gov\nist\microanalysis\PyEPQ\<Subpkg>\tests\archive). Repair the ports in a sequential manner, without paralellization. Update the versioning scheme in accordance with the number of port repairs.
