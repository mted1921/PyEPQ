r"""
test_parity_multidhistogram_ver1_1_0.py — parity harness for MultiDHistogram_ver1_1_0.py
...
def _make_2d_histo():
    bins = PyLinearBins([0.0, 0.0], [1.0, 1.0], [10, 10])
...
class TestLinearBins:
    def test_dimensions_2d(self):
        lb = PyLinearBins([0.0, 0.0], [1.0, 1.0], [5, 5])
        assert lb.dimensions() == 2

    def test_dimensions_3d(self):
        lb = PyLinearBins([0.0] * 3, [1.0] * 3, [4, 4, 4])
        assert lb.dimensions() == 3

    def test_compute_origin_bin(self):
        lb = PyLinearBins([0.0, 0.0], [1.0, 1.0], [10, 10])
...
class TestMultiDHistogramParity:
    def test_parity_add_and_count(self):
        ...
        bins_py = PyLinearBins([0.0, 0.0], [1.0, 1.0], [5, 5])
