r"""
test_parity_adaptiverungekutta.py -- parity harness for AdaptiveRungeKutta_ver1.py
...
from AdaptiveRungeKutta_ver1 import AdaptiveRungeKutta as PyARK
...
class TestConstruction:
    ...
    def test_workspace_fields_none(self) -> None:
        ode = _SinCosODE(2)
        assert ode.mWs2 is None
        assert ode.mYErr is None
        assert ode.mQcYTemp is None
...
class TestSaveInterval:
    ...
    def test_save_interval_absolute_value(self) -> None:
        ...
        ode = _SinCosODE(2)
        ode.setSaveInterval(-math.pi / 4.0)
        assert ode.mSaveInterval == math.pi / 4.0
...
class TestAccessors:
    ...
    def test_clearSaveInterval_resets_to_sentinel(self) -> None:
        ...
        ode.setSaveInterval(0.5)
        ode.clearSaveInterval()
        assert ode.mSaveInterval == sys.float_info.max
